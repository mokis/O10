...
class messanger:
    ...
    def synch(self):
        ...
        while True:
            ...
            if msg[0] == "ping" and self.pub_sockets.has_key(msg[1]):
                #print msg, self.pub_sockets[msg[1]]
    ...
    def recv(self, name, split=True):
        ...
        if self.sub_sockets[name][0] in socks and \
           socks[self.sub_sockets[name][0]] == zmq.POLLIN:
            ...
            if name == "pantilt_feedback":
                print "pt", msg
            #if len(msg) > 0:
            #    print "zmq recv:", msg, msg.split()
    ...
    def send(self, msg, name):
        #socks = dict(self.poller.poll(0))
        #if self.pub_sockets[name][0] in socks and \
        #   socks[self.pub_sockets[name][0]] == zmq.POLLOUT and \
        #   time.time() - self.pub_sockets[name][2] > \
        #   self.pub_sockets[name][1]:
        if time.time() - self.pub_sockets[name][2] > self.pub_sockets[name][1]:
        #print "snd", msg, ".", self.pub_sockets[name][0]
            self.pub_sockets[name][2] = time.time()
            try:
                msg = self.pub_sockets[name][0].send(msg)
            except:
                print "errori@subpub", name, msg
        #print "ret", len(msg)
